...
- TikTokのリフレッシュトークンは365日で失効するため、期限の1ヶ月前を目安にリマインドする
  (アクセストークン自体は24時間ごとに自動更新されるため対象外)
- YouTubeのリフレッシュトークンは、OAuth同意画面が「テスト中」のままだと7日で失効する
  (`youtube-data-api-setup.md.md`参照)。本番公開審査が完了するまでは、
  自動投稿ではなく手動投稿の運用が続いている旨をdaily-logに明記し続ける
